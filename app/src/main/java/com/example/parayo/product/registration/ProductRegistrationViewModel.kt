...
class ProductRegistrationViewModel(app: Application) : BaseViewModel(app) {
    ...
    val imageIds: MutableList<Long?> = mutableListOf(null, null, null, null) // 2

    val productName = MutableLiveData("")
    val description = MutableLiveData("")
    val price = MutableLiveData("")
    val categories = MutableLiveData(categoryList.map { it.name }) // 3
    var categoryIdSelected: Int? = categoryList[0].id // 4

    val descriptionLimit = 500
    val productNameLimit = 40 // 5
    ...
    private fun onImageUploadResponse(response: ApiResponse<ProductImageUploadResponse>) {
        if (response.success && response.data != null) {
            ...
            toast(response.message ?: "알 수 없는 오류 발생했습니다.")
        }
    }
